...
import me.arboriginal.PlayerRider.PROptions.PREffect;
...
class PRUtils {
  ...
  static void broadcast(String key, CommandSender player, CommandSender duck) {
    ...
    if (!message.isEmpty())
      Bukkit.broadcastMessage(prepareMessage(message, player, duck));
  }
  ...
  static void consume(Player player, ItemStack item, String key) {
    if (PR.config.getBoolean("consume_items." + key)
  ...
  //TODO: Apply on the main vehicle + refresh when unmount
  ...
  static int effectAmplifier(Player player, PREffect effect) {
    ...
    if (effect.inc < 0)
      return Math.min(effect.max, value);
    else
      return Math.max(value, effect.max);
  }
